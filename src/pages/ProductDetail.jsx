...
const ProductPage = () => {
  ...
  return (
    <Grid container spacing={3} sx={{ p: 3, bgcolor: "#f5f5f5", width: "100%" }}>
      ...
      <Grid container spacing={3} pb={12}>
        {/* LEFT: Images */}
        <Grid size={{ xs: 12, md: 7 }}>          <Card sx={{ borderRadius: 2 }}>
          <CardMedia
            component="img"
            width={"100%"}
            height="500"
          ...
          <Grid container spacing={2} mt={1}>
            {(product?.product_images || []).map((item, i) => (
              <Grid size={{ xs: 4 }} key={i}>                <Card sx={{ borderRadius: 2 }}>
                <Box
                  sx={{
                    width: "100%",
      ...
      >
        <Grid container alignItems="center">
          <Grid size={{ xs: 6 }}>            <Typography fontWeight="bold">₹ {product?.cost_price || "N/A"}</Typography>
          ...
          <Grid size={{ xs: 6 }} textAlign="right">            {/* <Button
              variant="contained"
              color="error"
              size="large"
              sx={{ px: 4 }}
